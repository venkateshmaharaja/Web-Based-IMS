$(document).ready(function(){
	...
	$.ajax({
			...
			success : function(data){
			var choose="<option value=''>Choose The Customer</option>";
		...
		$("#sales_entry").click(function(){

			var invoice=$("#sales_entry_form").serialize();
			alert(invoice);
			if($("#customer").val()===""){
			...
			$.ajax({
				...
				method:"POST",
				//dataType:json,
				data:$("#sales_entry_form").serialize(),
				success:function(data){
					console.log(data);
					//alert(data);
					if(data==1){
					alert("SALES ENTERED");
						...
						$("#sales_entry_form").trigger("reset"); 
